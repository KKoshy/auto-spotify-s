...
def artist(artists):
    artist = artists.get_artist(
        artist_id=ArtistAPIConstants.artist_id,
...
class TestArtistAPI:
    def test_content_type_header(self, artist):
        ...
        assert (
            artist.headers.get("content-type")
            == ArtistAPIConstants.json_content
        )

    def test_cache_control_header(self, artist):
        log.info("Verifying cache control")
        assert (
            artist.headers.get("cache-control")
            == ArtistAPIConstants.cache_control
        )
...
class TestNegativeArtistAPI:
    @pytest.mark.parametrize(
        "artist_id,status_code,token,message",
        [
            (
                "",
            ...
            (
                ArtistAPIConstants.artist_id[:-3] + "abc",
                ResponseConstants.status_404,
                None,
                "Resource not found",
            ),
            (
                ArtistAPIConstants.artist_id,
                ResponseConstants.status_401,
                "asdoinsadnfasoidnsione",
    ...
    def test_negative_case(
        self, artists, artist_id, token, status_code, message
    ):
        ...
        artist = artists.get_artist(
            artist_id=artist_id, ignore_handle_response=True
        )
