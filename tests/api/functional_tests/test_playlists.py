...
class TestPlaylistsAPI:
    ...
    def test_follow_playlist(self, playlists: PlaylistsAPI, users: UsersAPI):
        ...
        users.follow_playlist(
            playlist_id=PlaylistAPIConstants.nightly_playlist_id,
    ...
    def test_add_playlist_items(self, playlists: PlaylistsAPI):
        ...
        add_items = playlists.add_playlist_items(
            playlist_id=PlaylistAPIConstants.nightly_playlist_id,
        ...
        current_user_items = playlists.get_playlist_items(
            playlist_id=PlaylistAPIConstants.nightly_playlist_id,
            status_code=ResponseConstants.status_200,
            json_schema=PlaylistAPIConstants.playlist_items_schema,
        )
        uris = [
            item["track"]["uri"] for item in current_user_items.json()["items"]
        ]
        assert set(PlaylistAPIConstants.add_items_payload["uris"]).issubset(
            set(uris)
        )
    ...
    def test_remove_playlist_items(self, playlists: PlaylistsAPI):
        ...
        remove_items = playlists.remove_playlist_items(
            playlist_id=PlaylistAPIConstants.nightly_playlist_id,
        ...
        assert set(PlaylistAPIConstants.add_items_payload["uris"]).isdisjoint(
            set(uris)
        )
