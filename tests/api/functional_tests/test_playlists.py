...
class TestPlaylistsAPI:
    ...
    def test_follow_playlist(self, playlists: PlaylistsAPI, users: UsersAPI):
        ...
        follow = users.follow_playlist(playlist_id=PlaylistAPIConstants.nightly_playlist_id, 
    ...
    def test_add_playlist_items(self, playlists: PlaylistsAPI):
        ...
        assert add_items.status_code == 201
        global SNAPSHOT_ID
        SNAPSHOT_ID = add_items.json().get('snapshot_id')
        current_items = playlists.get_playlist_items(playlist_id=PlaylistAPIConstants.nightly_playlist_id)
    ...
    def test_remove_playlist_items(self, playlists: PlaylistsAPI):
        ...
        assert remove_items.status_code == 200
        current_items = playlists.get_playlist_items(playlist_id=PlaylistAPIConstants.nightly_playlist_id)
        assert current_items.status_code == 200
    ...
    def test_unfollow_playlist(self, playlists: PlaylistsAPI, users: UsersAPI):
        ...
        unfollow = users.unfollow_playlist(playlist_id=PlaylistAPIConstants.nightly_playlist_id)
